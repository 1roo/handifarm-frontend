...
const MarketDetail = () => {
    ...
    fetch(`${API_BASE_URL}/api/market/${itemNo}`, {
      ...
      if (!res.ok) {
        if (res.status === 403)
          ...
          alert("내부 코드 오류가 발생하였습니다. 관리자에게 문의 바랍니다.");
  ...
  const delBtn = () => {
    ...
    if (
      window.confirm(
        `'${thisItem.itemName}' 판매를 중단하시겠습니까?\n삭제된 게시글은 복구가 불가능합니다.`
      )
    ) {
      const requestHeader = {
        "content-type": "application/json",
          ...
          <div className="content">
            ...
            <Grid className="content-content">
              {thisItem.done ? ( //판매 여부에 따라 출력 구분
                <Button className="green-btn red" variant="contained" disabled>
                  {" "}
                  판매 완료{" "}
                </Button>
              ) : (
              ...
              <div className="title">
                <strong>{thisItem.itemName}</strong>{" "}
                <span>{thisItem.regDate}</span>
              </div>
              {/* <div className="score">
                <Rating
